...
> Version 2.0 alpha is in `develop` branch now. Hard to believe, but it's several times faster than the current version in all browsers. Designed with ES6 classes support, and ability to handle 10K collections of complex objects with subsecond real-time response in all browsers. Stay tuned.
